...
## Table of Contents
<!-- $toc-max-depth=2 -->
* [@continuous-labs/sdk](#continuous-labssdk)
...
```bash
npm add @continuous-labs/sdk
```

### PNPM

```bash
pnpm add @continuous-labs/sdk
```

### Bun

```bash
bun add @continuous-labs/sdk
```

### Yarn

```bash
yarn add @continuous-labs/sdk
```

> [!NOTE]
> This package is published as an ES Module (ESM) only. For applications using
> CommonJS, use `await import("@continuous-labs/sdk")` to import and use this package.
...
| ------------ | ---- | ----------- | ------------------------- |
| `apiKeyAuth` | http | HTTP Bearer | `CONTINUOUS_API_KEY_AUTH` |
...
* [listSimulations](docs/sdks/simulations/README.md#listsimulations) - List Simulations
...
* [listSimulators](docs/sdks/simulators/README.md#listsimulators) - List Simulators
...
* [listWorlds](docs/sdks/worlds/README.md#listworlds) - List Worlds
...
[`ContinuousError`](./src/models/errors/continuous-error.ts) is the base class for all HTTP error responses. It has the following properties:
...
```typescript
import { Continuous } from "@continuous-labs/sdk";
import * as errors from "@continuous-labs/sdk/models/errors";
...
async function run() {
  try {
    ...
    if (error instanceof errors.ContinuousError) {
...
**Inherit from [`ContinuousError`](./src/models/errors/continuous-error.ts)**:
...
const continuous = new Continuous({
  serverURL: "https://api.continuouslabs.ai",
  apiKeyAuth: process.env["CONTINUOUS_API_KEY_AUTH"] ?? "",
});
...
import { HTTPClient } from "@continuous-labs/sdk/lib/http";
...
const sdk = new Continuous({ httpClient: httpClient });
```
...
const sdk = new Continuous({ debugLogger: console });
```
...
### SDK Created by [Speakeasy](https://www.speakeasy.com/?utm_source=@continuous-labs/sdk&utm_campaign=typescript)
